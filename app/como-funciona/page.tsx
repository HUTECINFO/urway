...
export const metadata: Metadata = {
  title: "Cómo elegimos una oferta",
  description: "Conoce el criterio de UR WAY para separar un precio bajo de una oferta que realmente vale el viaje.",
  alternates: { canonical: "/como-funciona" },
};

const factors = [
  { icon: BadgeDollarSign, name: "Precio", weight: 40, copy: "Comparamos la oferta contra el rango real de una ruta, no contra un descuento publicitario." },
  ...
  { icon: Map, name: "Destino", weight: 15, copy: "Valoramos qué tan especial es la oferta según el destino y la dificultad de encontrarla." },
  ...
  { icon: Plane, name: "Calidad del vuelo", weight: 10, copy: "Escalas, duración y horarios importan: una oferta deja de ser buena cuando el trayecto no compensa." },
];
      ...
      <main>
        <section className="bg-sand pb-20 pt-32 sm:pb-32 sm:pt-44">
          <div className="container-page">
            <p className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.19em] text-coral"><Radar aria-hidden="true" className="size-4" /> Cómo elegimos</p>
            <h1 className="mt-5 max-w-6xl text-balance font-display text-[clamp(3.15rem,14vw,9.5rem)] font-extrabold leading-[0.84] tracking-[-0.075em] text-midnight sm:mt-6 sm:leading-[0.78] sm:tracking-[-0.085em]">Un precio bajo llama la atención.<br /><span className="text-coral">Una buena oferta pasa el filtro.</span></h1>
            <p className="mt-6 max-w-2xl text-base leading-7 text-slate sm:mt-9 sm:text-xl sm:leading-8">Convertimos precio, ahorro, destino, fechas y calidad de vuelo en una recomendación clara. Si no la elegiríamos para nosotros, no la publicamos.</p>
          </div>
        </section>
        ...
        <section className="bg-midnight py-20 text-white sm:py-32" aria-labelledby="score-heading">
          <div className="container-page grid gap-12 sm:gap-16 lg:grid-cols-[0.68fr_1.32fr] lg:gap-24">
            <div className="lg:sticky lg:top-32 lg:self-start">
              ...
              <h2 id="score-heading" className="mt-4 font-display text-[3rem] font-extrabold leading-[0.9] tracking-[-0.07em] sm:mt-5 sm:text-7xl sm:leading-[0.86] sm:tracking-[-0.075em]">100 puntos.<br />Cinco factores.</h2>
              <p className="mt-7 max-w-md leading-7 text-white/52">El precio y el ahorro pesan más. Los demás factores evitan que una tarifa barata esconda un viaje que no compensa.</p>
            </div>
        ...
        <section className="bg-white py-20 sm:py-32">
          <div className="container-page">
            <div className="grid gap-4 sm:gap-8 lg:grid-cols-3">
              ...
              <article className="rounded-[1.75rem] bg-coral p-6 sm:rounded-[2.25rem] sm:p-10"><span className="flex size-12 items-center justify-center rounded-2xl bg-midnight text-white sm:size-14"><Plane aria-hidden="true" /></span><p className="mt-7 text-[10px] font-extrabold uppercase tracking-[0.16em] text-midnight/55 sm:mt-10 sm:text-xs">03 · Tú decides</p><h2 className="mt-3 font-display text-[1.75rem] font-extrabold leading-tight tracking-[-0.05em] text-midnight sm:text-3xl sm:tracking-[-0.055em]">Te damos claridad, no presión.</h2><p className="mt-4 text-sm leading-6 text-midnight/60 sm:text-base sm:leading-7">Te mostramos el contexto y te llevamos a comparar agencias. La decisión final siempre es tuya.</p></article>
            </div>
            <div className="mt-10 flex flex-col gap-6 border-t border-midnight/10 pt-8 sm:mt-14 sm:flex-row sm:items-center sm:justify-between sm:pt-10">
              <p className="max-w-xl font-display text-[1.8rem] font-extrabold leading-tight tracking-[-0.05em] text-midnight sm:text-3xl sm:tracking-[-0.055em]">Ya conoces el criterio. Ahora encuentra una oferta que te haga querer despegar.</p>
              <Link href="/drops" className="inline-flex min-h-13 w-full shrink-0 items-center justify-center gap-2 rounded-2xl bg-midnight px-6 text-sm font-extrabold text-white transition hover:bg-coral hover:text-midnight sm:w-auto">Explorar ofertas <ArrowRight aria-hidden="true" className="size-4" /></Link>
            </div>
          </div>
        </section>
      </main>
