...
import { BarChart3, ExternalLink, FileSearch, LayoutDashboard, LogOut, PlaneTakeoff, Sparkles } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#f6f5f1] text-midnight lg:grid lg:grid-cols-[17rem_minmax(0,1fr)]">
      <aside className="hidden h-screen flex-col border-r border-white/10 bg-midnight px-5 py-6 text-white lg:sticky lg:top-0 lg:flex">
        <Link href="/admin" className="flex items-center gap-3 px-2" aria-label="UR WAY Admin">
          <span className="flex size-10 items-center justify-center rounded-full bg-white text-midnight"><PlaneTakeoff size={18} /></span>
          <span>
            <span className="block font-display text-lg font-extrabold tracking-[-0.055em]">UR WAY</span>
            <span className="block text-[9px] font-bold uppercase tracking-[0.22em] text-white/40">Editorial desk</span>
          </span>
        </Link>
        ...
        <div className="mt-auto">
          <div className="mb-4 rounded-2xl border border-white/10 bg-white/5 p-4">
            <Sparkles className="mb-3 text-sky" size={18} />
            <p className="text-sm font-semibold">Selección inteligente</p>
            <p className="mt-1 text-xs leading-5 text-white/45">Cada score ayuda a decidir qué merece convertirse en Drop.</p>
          </div>
        ...
        <header className="sticky top-0 z-30 border-b border-midnight/8 bg-[#f6f5f1]/90 backdrop-blur-xl">
          <div className="flex h-18 items-center justify-between px-4 sm:px-7 lg:px-10">
            <Link href="/admin" className="flex items-center gap-2.5 lg:hidden">
              <span className="flex size-9 items-center justify-center rounded-full bg-midnight text-white"><PlaneTakeoff size={16} /></span>
              <span className="font-display font-extrabold tracking-[-0.05em]">UR WAY</span>
            </Link>
            <div className="hidden lg:block">
              <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-slate">Operations</p>
              <p className="text-sm font-semibold text-midnight">Centro de control editorial</p>
            </div>
