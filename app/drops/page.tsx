...
export const metadata: Metadata = {
  ...
  description: "Explora ofertas de vuelo con buen precio, buena ruta y contexto claro desde tu aeropuerto de salida.",
      ...
      <main>
        <section className="relative overflow-hidden bg-midnight pb-16 pt-28 text-white sm:pb-28 sm:pt-40">
          ...
          <div className="container-page relative grid gap-8 sm:gap-10 lg:grid-cols-[1fr_23rem] lg:items-end">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[0.19em] text-coral">Ofertas desde México</p>
              <h1 className="mt-4 max-w-5xl text-balance font-display text-[clamp(3.3rem,15vw,9rem)] font-extrabold leading-[0.84] tracking-[-0.075em] sm:mt-5 sm:leading-[0.78] sm:tracking-[-0.085em]">Buenas ofertas.<br /><span className="text-coral">Cero relleno.</span></h1>
              <p className="mt-5 max-w-2xl text-base leading-7 text-white/58 sm:mt-7 sm:text-lg sm:leading-8">Cada oferta pasó por el mismo filtro. Elige tu salida, revisa los detalles y compara solo las rutas que realmente valen la pena.</p>
            </div>
        ...
        <section id="drops" className="scroll-mt-24 bg-background py-16 sm:py-28" aria-labelledby="catalog-title">
          <div className="container-page">
            <div className="flex flex-col gap-5 border-b border-midnight/10 pb-6 sm:gap-7 sm:pb-8 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <p className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.17em] text-coral"><SlidersHorizontal aria-hidden="true" className="size-4" /> Elige tu salida</p>
                <h2 id="catalog-title" className="mt-3 font-display text-[2.5rem] font-extrabold leading-[0.95] tracking-[-0.06em] text-midnight sm:text-6xl">{deals.length} {deals.length === 1 ? "oferta" : "ofertas"}{selectedAirport ? ` desde ${selectedAirport.city}` : " activas"}.</h2>
              </div>
            ...
            <nav className="-mx-5 mt-6 flex gap-2 overflow-x-auto px-5 pb-3 [scrollbar-width:none] sm:mx-0 sm:mt-7 sm:px-0" aria-label="Filtrar por tipo de oferta">
            ...
            {deals.length > 0 ? (
              ...
              <div className="mt-8 rounded-[1.75rem] bg-white p-6 text-center shadow-[0_22px_70px_rgba(13,27,42,.07)] sm:mt-10 sm:rounded-[2.5rem] sm:p-14">
                ...
                <p className="mx-auto mt-4 max-w-xl leading-7 text-slate">Prueba otro filtro o recibe un aviso cuando aparezca una oferta que encaje contigo.</p>
        ...
        <section className="bg-coral py-16 sm:py-28" aria-labelledby="drops-newsletter-title">
          <div className="container-page grid gap-8 lg:grid-cols-[1fr_0.85fr] lg:items-end">
            <div><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-midnight/55">Las buenas ofertas no esperan</p><h2 id="drops-newsletter-title" className="mt-4 max-w-4xl font-display text-[2.85rem] font-extrabold leading-[0.92] tracking-[-0.065em] text-midnight sm:text-7xl sm:leading-[0.88] sm:tracking-[-0.07em]">Recíbelas antes de que desaparezcan.</h2></div>
